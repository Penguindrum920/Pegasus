...
export const calculateInitialPosition = (isEntry = false, customDistance = null) => {
    const theta = Math.random() * Math.PI * 2
    const radius = customDistance || (isEntry ? SPAWN_RADIUS * 1.5 : Math.random() * SPAWN_RADIUS + SUN_RADIUS * 3)
    const x = Math.cos(theta) * radius
    const y = 0 // Keep planets on the same plane for circular orbits
